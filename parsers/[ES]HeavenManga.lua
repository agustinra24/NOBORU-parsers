...
function HeavenManga:getManga(h,i)
	local j=e(h):gsub("\n"," "):gsub("\r"," ")
	i.NoPages=true
	for k,l,m in j:gmatch('class="manga%-name">%s*([^<]-)%s*<%/div>.-href="https?://.-%.com(/%a[^"]-)".-src=\'([^\']-)\'')do
		i[#i+1]=CreateManga(a(k),l,m,self.ID,self.Link..l)
		i.NoPages=false
	end
end
...
function HeavenManga:searchManga(p,n,i)
	...
	i.NoPages=true
	for m,l,k in j:gmatch('class="page%-item%-detail".-src=\'([^\']-)\'.-href="https?://.-%.com(/%a[^"]-)".-class="manga%-name">%s*([^<]-)%s*<%/div>')do
		i[#i+1]=CreateManga(a(k),l,m,self.ID,self.Link..l)
	end
end

function HeavenManga:getChapters(q,i)
	local j=e({Link=self.Link..q.Link,Header1="X-Requested-With: XMLHttpRequest"})
	local r={}
	if j:find('"slug":',1,true)then
		for l,k in j:gmatch('"slug":"([^"]-)".-<i>%s*([^<]-)%s*<%/i>')do
			r[#r+1]={Name=k,Link="/"..l,Pages={},Manga=q}
		end
	elseif j:find('"data":',1,true)then
		for l,k,n in j:gmatch('"number":"([^"]-)".-"(%-)":"([^"]-)"')do
			r[#r+1]={Name=k.." "..n,Link="/"..q.Link.."/"..l,Pages={},Manga=q}
		end
	end
	if#r==0 and j:find('<a href=".-/manga/.-/%d',1,true)then
		local s=j:gsub("\n"," "):gsub("\r"," ")
		for l,k in s:gmatch('<a[^>]-href=".-(/manga/[^/]-/%d[^"]-)".->(.-)</a>')do
			r[#r+1]={Name=k,Link=l,Pages={},Manga=q}
		end
	end
	table.sort(r,function(c,s)
		local t=tonumber(c.Name:match("([0-9.,]*)$")or"")
		local u=tonumber(s.Name:match("([0-9.,]*)$")or"")
		if t~=nil and u~=nil then return t<u end
...
function HeavenManga:prepareChapter(w,i)
	...
	local x=j:match('href="[^"]-(/manga/leer/[^"]-)".-rel="nofollow"')or""
	if x==""then
		x=j:match('href="([^"]-/manga/[^/]-/leer/[^"]-)"')or""
		j=e(self.Link..x)
	end
	if x~=""then
		j=e(self.Link..x)
		for l in j:gmatch('"imgURL":"([^"]-)"')do
			i[#i+1]=l
		end
	end
	if#i==0 then
